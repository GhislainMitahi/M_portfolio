...
for (let i = 0; i < works.length; i += 1) {
  ...
  for (let j = 0; j < works[i].technologies.length; j += 1)
    tech += `<li> ${works[i].technologies[j]} </li>`;
...
function popop(e) {
  ...
  let tecks = "";
  for (let j = 0; j < currentWork.popopDescription.technologies.length; j += 1)
    tecks += `<li class = 'popop-li'>${currentWork.popopDescription.technologies[j]}</li>`;
